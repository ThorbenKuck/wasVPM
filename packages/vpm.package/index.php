...
function free_days_to_array ($jahr) {
	if (!empty ($_SESSION['VPM_FREE_DAYS'])) {
		...
		\Main\top_level_debug ("vpm.package", "(Error)Die Datei \"days.php\" wurde nicht gefunden. Die Mod-configuration konnte nicht geladen werden!", true);
		return null;
	}
}
...
function find_core_path ($path = null) {
	...
	if (!empty($_SESSION['PATH_TO_VPM_CORE'])) {
		...
		if (isset(\Main\info ()['mod_folder_root_path'])) {
			// This is a hard-coded fallback!
			// If, at any time, this is used, you get notified in the debug-frame
			$path = \Main\info ()['mod_folder_root_path'] . "vpm.mod/core/core.csv";
			\Main\top_level_debug ("vpm.package", "(Notice)the function \"find_core_path\" used it's fallback!" . "<br>" . $path . "<br>" . "This means, it relies on the internal assumptions of the estimated path..", true);
		} else {
			\Main\top_level_debug ("vpm.package", "(Error)Die Datei \"core.csv\" wurde nicht gefunden", true);
			return null;
		}
	}
	...
	if (file_exists ($path)) {
		...
		\Main\top_level_debug ("vpm.package", "(Error)with " . $path . "!<br>" . "Either this file exists, or this file is not readable!", true);
		return false;
	}
}
...
function core_array ($path = null) {
	...
	if ($path !== null) {
		if (file_exists ($path) && basename ($path) == "core.csv") {
			...
			\Main\top_level_debug ("vpm.package", "(Warning)You have given the function \"core_array\" the path = \".$path.\"." . "<br>" . " This is not a valid path for the \"core.csv\"-file!", true);
			return core_array ();
		}
	} else if (isset($_SESSION['core_path'])) {
		$core_file = file ($_SESSION['core_path']);
	} else {
		if (file_exists (find_core_path ())) {
			$core_file = file (find_core_path ());
		} else {
			\Main\top_level_debug ("vpm.package", "(Error)while opening \"" . find_core_path () . "\"!", true);
			return [];
		}
	}
	if ($core_file === null) {
 ...
 * @param null $path
 * @return bool succsefully_insert_date
 */
...
function get_normal_day_state () : bool{
	...
	if (!defined ('STARTUHR') || !defined ('ENDUHR')) {
		echo "Es wurde ein Erheblicher Fehler in der Berechnungsroutine fest gestellt!";
		\Main\log("In der Berechnungs-routine \"Main\\get_normal_day_state\" fehlen die Konstanten \"STARTUHR\" und \"ENDUHR\"");
	}
...
function get_vpm_config () {
	\Main\read_config ();
	return require \Main\info ()['mod_folder_root_path'] . "vpm.mod/config.php";
}
